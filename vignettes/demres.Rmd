...
# Introduction of the framework of demres
...
To date, most applications of demographic resilience have focused on comparative analyses, where demographic rates are _standardized_ and _averaged over time_ to facilitate comparison across species or populations.

# From comparative to applied demographic resilience

The original demographic resilience framework standardizes matrix population models (MPMs) and demographic structures in order to isolate transient dynamics and allow comparisons across populations (Stott et al. 2011[^4]; Capdevila et al. 2020[^1]). While essential for comparative studies, such standardization may be insufficient for applied questions.

![Destandardizing resilience measures.](figures/figure1.png)
**Figure 1.** Conceptual workflow of the demres package. 
...
- assess resilience in response to actual, historically observed disturbances.

Moreover, demographic rates in wild populations are rarely constant. Long-term studies demonstrate substantial temporal variation driven by environmental variability, anthropogenic pressures, and episodic disturbance events. Averaging demographic rates across time can therefore overlook the signals that are most relevant for management.

![Time-varying deomgraphic rates.](figures/figure2.png)
![Destandardizing resilience measures.](figures/figure3.png)

The demres package adopts an explicitly applied perspective by quantifying resilience at each time step, using the demographic rates observed at these time steps. In doing so, demres provides a phenomenological approach to identifying years associated with unusually high or low resilience, which may correspond to disturbance events or favorable conditions.

The R package `demres` was developed to address this need. It extends _transient demographic analysis_ to allow the quantification of time-varying demographic resilience, thereby leveraging temporal variation in demographic data rather than averaging it. This vignette introduces the conceptual background of `demres` and provides a step-by-step guide to its application in conservation and management contexts.
...
1- Specify the demographic structure of the population

2- Project population dynamics basedd on the demographic structure
...
[^5]: [Hinke et al. 2017 Ecosphere, 8(1), e01666.](https://esajournals.onlinelibrary.wiley.com/doi/epdf/10.1002/ecs2.1666)
...
# Plot convergence time value over each time step 
plot(
  AP_demres_stand$timestep,
  AP_demres_stand$convt,
  type = "l",                   
  main = "Convergence time value over time",
  xlab = "Time Step",
  ylab = "Convt",
  col = "black",                
  lwd = 2)
...
# Plot the damping ration calculated for each time step
plot(
  AP_demres_stand$timestep,
  AP_demres_stand$dr,
  type = "l",                   
...
While the year 26 is the one with the lowest value of reactivity, with a value of 0.9113. The population abundance during that year would be 52.14 individuals. So inferior to the starting abundance. This means that in addition to attenuating, the population is decreasing. 
