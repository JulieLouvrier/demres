...
# df <- map_dfr(seq_along(adeliepenguin), function(i) {
#   mat <- adeliepenguin[[i]]
#   data.frame(
#     index = i,
#     fertility = mat[1,2],
...
#     Trja = mat[2,1]
#   )
# }) %>% pivot_longer(cols = c(fertility, Sa, Trja), names_to = "Demographic_parameter")


# # Plot using ggplot
# ggplot(df, aes(x = index, y = value, color = Demographic_parameter)) +
#   geom_line() +
#   labs(title = "Demographic parameters over time",
#        x = "Time Step",
#        y = "Demographic parameter",
#        color = "Parameter") +
#   theme_minimal() 
...
##DON'T FORGET A SECTION ON INITIAL VECTOR
...
# Plot using ggplot
# ggplot2::ggplot(AP_demres_reac_stand, aes(x = timestep, y = reac)) +
#   geom_line() +
#   labs(title = "Stadardized reactivity over time",
#        x = "Time Step",
#        y = "Reactivity") +
#   theme_minimal() +
...
Another way to visualize it is through violin plots 
##HERE STOP
...
NEXT: It would still be good to show all the functionality of plotting, without facettig and with; with scaling by both lambda and pop vector or only by lambda
**Vik: I think it may be a good idea to compute lambda for each matrix and show it on the plot that shows both unstandardized lambda and pop vec?? - or at least for now for the workshop to show it separately - how majority of the pops are projected to increase and only a few to decline according to the intrisic rate of increase**
...
## Vik: HERE still add short discussion of the variation observed + example of calculating coeff. of variation

**Vik - note: I guess all of this below will be deleted now? So for now I just shifted the plot functionality to above - as I think it is important to show how plots work as soon as possible within the vignette - I have a feeling seeing it all makes things much clearer than trying to explain it with words at length. Please revert whatever you feel is best for the workflow of the hands-on and vignette more broadly**
