...
The original demographic resilience framework standardizes matrix population models (MPMs) and demographic structures in order to isolate transient dynamics and allow comparisons across populations (Stott et al. 2011[^4]; Capdevila et al. 2020[^1]). Standardizing consists in (i) standardizing the matrix A by dividing A with its long term asymptotic growth rate ${\lambda_{max}}$ to detangle log term asymptotic behavior to the transient behavior, and (ii) standardizing the initial population structure ${\textbf{n}}_0$ with its total sum, to compare resilience between populations. While essential for comparative studies, such standardization may be insufficient for applied questions.
...
**Figure 3.** Illustration of projections and demographic resilience measures from a population that has demographic rates varying in time. On the left panel are the measures based on the standardized approach while on the right panel are the same measures based on the destandardized approach.

The R package `demres` package adopts an explicitly applied perspective by quantifying resilience at each time step, using the demographic rates observed at these time steps. In doing so, demres provides a phenomenological approach to identifying years associated with unusually high or low resilience, which may correspond to disturbance events or favorable conditions. `demres` was developed to address this need. It extends _transient demographic analysis_ to allow the quantification of time-varying demographic resilience, thereby leveraging temporal variation in demographic data rather than averaging it. This vignette introduces the conceptual background of `demres` and provides a step-by-step guide to its application in conservation and management contexts.

[^1]: [Capdevila et al. 2020 Trends Ecol Evol, 35(9), 776-786.](https://pubmed.ncbi.nlm.nih.gov/32482368/)

[^2]: [Capdevila et al. 2022 Ecol letters, 25(1), 240-251.](https://onlinelibrary.wiley.com/doi/full/10.1111/ele.14004)

[^3]: [Cant et al. 2023 Ecol Letters, 26(7), 1186-1199.](https://onlinelibrary.wiley.com/doi/full/10.1111/ele.14234)

[^4]: [Stott et al. 2011 Ecol Letters, 14(9), 959-970.](https://onlinelibrary.wiley.com/doi/full/10.1111/j.1461-0248.2011.01659.x)
...
2- Project population dynamics based on the demographic structure
...
The different resilience metrics that are calculated can be :         
...
- _**Time to Target**_ : the number of timesteps in the projection necessary to reach a target population abundance.
...
The function `resilience` can compute each of the aforementioned metrics, calculated for each matrix that was made at each time step of the period of the study. This function by default does not standardise neither the matrix (by dividing it by ${\lambda_{max}}$) nor does it scale the population vector to sum up to 1. These two standardisations were necessary within the comparative framework of demographic resilience as introduced by Capdevilla et al. (2020). The user can apply each standardisation by setting the respective argument `return.N` to `FALSE` (see below for details). 
...
Given the population demographic structure specified in ${n_0}$, it appears
...
For example, let us compute mean and standard deviation of all computed resilience metrics using the user-supplied demographic structures for each year.
