...
# Step 1: Extract data from matrices into a data frame
...
# Step 2: Convert from wide to long format manually
long_df <- rbind(
...
# Empty plot setup
plot(
  NA, xlim = range(long_df$index), ylim = range(long_df$value),
...
for (i in seq_along(params)) {
  ...
  lines(param_data$index, param_data$value, col = colors[i], lwd = 2)
}

# Add legend
legend("topright", legend = params, col = colors, lty = 1, lwd = 2, title = "Parameter")
...
```{r, reac plot echo = -1, fig.width = 6, fig.height = 6}
# Plotting reactivity over time 
# Plot using ggplot
# First, make sure your data is sorted by timestep
...
It is therefore important to keep in mind that each metric is calculated relative to the asymptotic growth rate. It is possible to compute it by using the function __{eigs}__ from the package __{popdemo}__ 
...
As for the standardized reactivity, the maximum population abundance for __{reactivity}__ calculated with the provided vector was found during the 14th year of the study period, with a value of 79.26 individuals. This is the projected number of indiviuals when the population reached its maximum reactivity. And the minimum value is reached during year 7 with 50.64 individuals. 

```{r, reac abundance plot}
# Plotting demographic resilience metrics over time 
# Plot using ggplot
# ggplot2::ggplot(AP_demres_reac, aes(x = timestep, y = reac)) +
#   geom_line() +
#   labs(title = "Reactivity over time",
#        x = "Time Step",
#        y = "Reactivity") +
#   theme_minimal() +
...
# Plotting demographic resilience metrics over time 
# ggplot2::ggplot(AP_demres_stand, aes(x = timestep, y = dr)) +
#   geom_line() +
#   labs(title = "Damping ratio over time",
#        x = "Time Step",
#        y = "DR") +
#   theme_minimal() +
#   geom_hline(yintercept = 1, col = "red")
# 
# ggplot2::ggplot(AP_demres_stand, aes(x = timestep, y = convt )) +
#   geom_line() +
#   labs(title = "Convergence time over time",
#        x = "Time Step",
#        y = "convt") +
#   theme_minimal() 
...
In our case, __{reactivity}__ is equal to maxamp for the year 14 and equal to maxatt for the rest of the years. So the plot of __{reactivity}__ would be enough. 
...
While the year 26 is the one with the lowest value of reactivity, with a value of 0.9113. The population abundance during that year would be 52.14 individuals. So inferior to the starting abundance. This means that in addition to attenuating, the population indeed is decreasing. 
...
The first advantage of `demres`is its adaptability to historically informed data from the field. If demographers have the information of the population abundance in each respective (st)age class for each timestep of the study, it is possible to integrate it in the framework
...
## Same repeated value
This is the example we haveb een using in the beginning of this vignette 
```{r, repeat vector}
# simulate an initial vector
set.seed(125435)
vec <- round(runif(2) * 100 )
...
# Plot 
The function `plot` provides a visual representation of the populations trajectories obtained when starting the projection from each supplied matrix and initial vector. Let us have a look at what trajectories look like when not standardising calculation of demographic resilience metrics by scaling the matrix entry by lambda and by scaling the population vector to sum to 1.
...
It is possible to order them by decreasing order
...
For example, let us compute mean, standard deviation and variance of all computed reisilience metrics using the user-supplied demographic structures for each year.
...
While the sd of damping ratio remain low in comparison with the other values, but it is because values of damping ratio are not expeted to varie as much as population abundances. To be able to account for the scaling issue of some of these parameters 
