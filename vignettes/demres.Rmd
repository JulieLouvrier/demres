...
knitr::opts_chunk$set(comment = "##  ", fig.align = "center", out.width="60%", message=FALSE)
library(demres)
options(digits = 4)
```
...
It is build around one dependency: [`popdemo`](https://github.com/iainmstott/popdemo).
...
# Introduction to the original framework on demographic resilience
...
The original demographic resilience framework is based on two elements: a Matrix Population Model, denoted $\mathbf{A}$ and an initial demographic distribution, summarized in a vector, denoted ${\textbf{n}}_0$. 

The elements of $\mathbf{A}$ (known as vital rates) define, per unit of time, survival rates in a given (st)age, transition probabilities from one (st)age to another, and fecundity (i.e. per-capita number of offspring that is contributed to the population by each (st)age).
...
The original demographic resilience framework standardizes matrix population models (MPMs) and initial demographic distribution in order to isolate transient dynamics (from long-term asymptotic dynamics) and allow comparisons across populations or species (Fig. 1, Stott et al. 2011[^4]; Capdevila et al. 2020[^1]). Standardizing consists in:

 (i) standardizing the matrix  $\mathbf{A}$ by dividing  $\mathbf{A}$ by its long-term asymptotic growth rate ${\lambda_{1}}$ to disentangle long-term asymptotic dynamics from transient dynamics, and  
 (ii) standardizing the initial demographic distribution ${\textbf{n}}_0$ by the total abundance, to enable comparison of resilience among populations that differ in their initial population sizes (Fig. 1). While essential for comparative studies, such standardizations may not be needed or even misleading for applied questions. 

The resulting standardised matrix is then denoted $\hat{\mathbf{A}}$ and the standardized initial vector ${\hat{\textbf{n}}_0}$. 
  
To assess demographic resilience using these two elements, the population is then projected and its transient behaviour is quantified before it reaches asymptotic growth. This is implemented in the `popdemo` package (Stott et al. 2012 [^5]), which allows extracting the following demographic resilience metrics: 

- _**Convergence time**_ : Time to convergence of a population to asymptotic growth rate obtained from the model projection. Convergence time is obtained by projecting the dynamics with the given model and determining the time point when convergence to a given accuracy is reached       
...
- _**Reactivity**_ : The population density that can be reached in the first time step after disturbance. Calculated as  

$$\textit{Standardized reactivity} = \hat{{\rho}_1} = ||\hat{\textbf{A}} \hat{\textbf{n}}_0||_1$$

With $\textbf{A}$ the matrix population model and $\hat{\textbf{A}}$ the standardized population matrix which is calculated as $\hat{\textbf{A}} = \frac{\textbf{A}}{\lambda_{1}}$. The vector $\hat{\textbf{n}}_0$ represents the initial demographic distribution, standardized to sum to 1 and is calculated as $\hat{\textbf{n}}_0 = \frac{{\textbf{n}}_0}{||\textbf{n}_0||_1}$. 

With ${||\textbf{m}||_1}$ representing the one-norm of a vector ${\textbf{m}}$ (i.e. sum of the absolute values of its entries)

- _**Maximal amplification**_ : The largest population density that can be reached at any time after disturbance. This metric is calculated using:
$$Standardized\ maximal\ amplification = \hat{{\rho}}_{max} = \max_{t > 0}||\hat{\textbf{A}}^t\hat{\textbf{n}}_0||_1$$

- _**Maximal attenuation**_ : The lowest population density that can be reached at any time after disturbance. Calculated as: 

$$Standardized\ maximum\ attenuation = \hat{{\rho}}_{min} = \min_{t > 0}||\hat{\textbf{A}}^t\hat{\textbf{n}}_0||_1$$
_Maximal amplification_ and _maximal attenuation_ can occur at any point after the disturbance over the course of projection. Some populations only have a _maximal amplification_ due to either their demographic rates or the initial vector (because they never attenuate), some only have a _maximal attenuation_ (because they never amplify), and some have both.


# From comparative framework to applied focus on demographic resilience


![Figure 1: Destandardizing resilience metrics in two steps:  Destandardizing allows to (i) go back from $\hat{\textbf{n}}_0$ (population density) to ${\textbf{n}_0}$ (population abundance) (second panel) and (ii) include long term asymptotic behavior into the demographic resilience measure (third panel). Each panel shows the trajectory of two examplary populations: one in cyan and one in blue. Red crosses on panels on the very left and on the very right show the resilience metrics (for notations see formulas above) with the panel on the left showing resilience metrics computed when two types of standardization is applied, and the panel on the right shows the metrics without standardization.](figures/figure1.png){width=130%}


For conservation and management, it is often critical to:

- estimate population abundance in terms of number of individuals, which is directly interpretable and often linked to management goals;

- assess not only transient dynamics per se (by stripping off the long-term population trend) but to look at the total dynamics (sometimes also called 'realised dynamics') that combines both the long-term and the short-term dynamics;

- assess demographic resilience in response to actual, historically observed disturbances.

Therefore, for conservation and management it is often more useful to compute a total (or "realised") dynamics in response to a disturbance, which means no standardization of either the population matrix or demographic distribuion is needed (Fig. 1).
...
![Figure 2. Representation of a time-varying demographic rate, here for illustration adult survival is shown.](figures/figure2.png){width=600px}


![Figure 3. Illustration of population projections and demographic resilience measures for a population with demographic rates that are varying in time. On the left-hand side shown are resilience metrics based on the "classical" standardized approach, while on the right-hand side the metrics based on the destandardized approach, i.e. realisedreflecting realised population dynamics. Each panel show metrics for a specific time step (3 or 4).](figures/figure3.png){width=130%}

The R package `demres` quantifies resilience at each time step, by using the demographic rates observed at these time steps. By doing so, demres provides a phenomenological approach to identifying years associated with unusually high or low resilience, which may correspond to disturbance events or favorable conditions. `demres` thus extends _transient demographic analysis_ in two ways: 

- to allow the quantification of time-varying resilience 
- to allow quantification of realised dynamics, i.e. focusing on resilience in terms of population abundance (instead of looking at densities, when the initial demographic distribution is standardized to sum to 1).  

Most importantly, `demres` also allows looking at either transient dynamics per se (when the long-term population trend is stripped off) or the overall, realised, dynamics (transient and long-term dynamics combined). We think the ability to look at these components separately and combined will assist decisions for conservation and management. This vignette introduces the conceptual background of `demres` and provides a step-by-step guide to its application in conservation and management contexts.
...
In technical terms, since maximal amplification and minimal attenuation correspond to, respectively, the maximum and minimum possible population density over the course of the projection, to be able to detect it correctly when the realised dynamics is computed (i.e. no standardisation is applied), the population is then projected and the population size is assessed at the time-step the "standardized" metrics are reached (for instance at the time-step 1 for reactivity).


# Study example: Adelie penguins
...
•	`vector`: a list of numeric vectors or one-column matrices describing the age/stage distribution ('initial demographic distribution') used to calculate a 'case-specific' resilience metric, i.e. based on the stage- or age-structure (as opposed to using a stage-biased vector to reflect a perturbation, a commonly used approach in transient analyses).
...
•	`popname`: Character string - the name of the population.
...
• `return.N`: Boolean. If set to `TRUE`, the metrics will be returned in their 'nonstandardised' form, i.e. the realised population dynamics will be used and the metrics will be in terms of the number of individuals. If set to `FALSE`, returns the standardized value of the metric. By default set to `TRUE`.
...
We can look at the stable age structure from the function `popdemo::eigs` and create a scenario in which the initial demographic distribution would differ from its stable structure. Here in that case we decided to have an under representation of juveniles and over representation of adults in a population of 60 individuals. **VIK: I do not understand what you mean by we decided to have  an underrepresentation of juveniles and overrepresentation of adults. An under- and over-representation wrt what? You mean we used the stable stage structure - that is you compute with 'ss' - but then it is not an under- or over-representation of any stage, it is the exact population vector that is observed when population growth rate is 1. So, I am confused what you want to say  and exactly do here**

```{r, stableAgeStructure, echo = -1}
#checking the ss 
do.call(rbind, lapply(adeliepenguin, popdemo::eigs, what = "ss"))*60
```
What we obtain is the list of stable structures based on the list of matrices, assuming that the population consists of 60 individuals. 

For the sake of illustration here, let us use the initial population structure with 15 juveniles and 45 adults. To apply this initial demographic structure to a list of our population matrices, we make a list where this vector is repeated hte same number of times as we have population matrices.
...
We can see that the standardized `reactivity` \hat{{\rho}_1} slightly varies in time. The _maximum value_ for the standardized calculation of `reactivity`, calculated with the provided initial vector was found during the _14th year_ of the study period. Year 14 is the year with the _highest_ value of fecundity (0.75). 

A `reactivity` of 1.1 would mean that in the first timestep, the population grows 1.1 as fast as a population that grows at its asymptotic growth rate. It is therefore slightly amplifying. However, we can also see that the _year 14_ is the only year for which the population amplifies. **VIK: I do not follow. There are several years with reactivity >1, so the year 14 is NOT the only year when the pop is amplifying. Or am I missing smth?** The _lowest value_ of reactivity reached in the _26th year_, with a value of 0.96. 

### Temporal variation of reactivity over 28 years
...
We see the variation among years in the standardised resilience values that is driven by variation in population matrices only. Thus, it is a key to keep in mind that each resilience metric in the original framework (where two types of standardization are applied) is calculated relative to the asymptotic population growth rate. In the framework we propose within `demres` we, by default, do not strip off asymptotic dynamics and thus look at the overall, realised population dynamics that combines both the short-term and the long-term population. To better understand what it means, let us compute the asymptotic population growth rate $\lambda_{max}$ using the population matrix for each year with the function `eigs` from the package `popdemo`.
...
```{r, plot lambda, echo = TRUE, fig.width = 6, fig.height = 6}
plot(
  eigs_AP$timestep,
  eigs_AP$lambda,
  type = "l",                   
...
We can see that the long term growth rate also varies in time. When it is above 1, it means that the population is growing while when it is below 1, it is declining. While it seems that for most of the years, the population would grow, we can look at the values of the long term growth rate during _year 26_ that has a value of 0.95. For this specific year, not only is the population declining but it is also attenuating, which means that is it growing even slower than its long term growth rate. **VIK: people have not read yet / checked yet attenuation, so it is somewhat premature and may be confusing to talk about these two things here. Maybe then bring this example after you have calculated all the metrics with realised population dynamics??**


## "Destandardized" or realised reactivity 

Let us now compute reactivity using the realised population dynamics, i.e. \hat{{\rho}_1} that does not use any of the standardizations. We achieve this by setting the argument `return.N = TRUE`.

```{r, realised reactivity}
# simulate an initial vector
vec <- c(15,45)
vector_list <- lapply(1:28, function(i) vec)
...
Just like for the standardized reactivity, the _maximum population abundance_ for `reactivity` calculated with the provided vector was found during the _14th year_ of the study period, with a value of 86.1 individuals. But this time, the _minimum value_ (across the study) is reached during _year 7_ with 52.80 individuals. _Year 14_ was the year with the _highest fecundity_ while _year 7_ was the year with the _lowest value_ of fecundity.  **VIK: I think it would b ehelpful if this plot had two panels, one for the 'realised reactivity' that you have just computed and one for the classical ('standardised') reactivity you have shown above. Having both panels next to each other will faciliate comparison and help following the text. Otherwise, the reader has to scroll the html up and down...**
...
**VIK: I think before we show the values of all the metrics computed on realised (or oevrall) dynamics we shall first explain them. For example, Time to target has not been defined anywhere so far. And, for all the others, I think ti may be useful to reiterate that the same formulas as specified for "hat"-versions ("standardised") are used, except that the pop matrix is not devided by lambda1 and the n0 is not scaled. So, I think it may be used to use the notation for these metrics without a hat. Or is there any other way we cna clearly refer to them (to make clear they are a distinct group)? Ella is thinking of using "realised reactivity" in her MS3 now, but we are still not 100% sure this is the best term. What are your thoughts on how to make crystall clear that these are not the original transient analyses metrics (as they compound both the long-term and the short-term dynamics, I revised the vignette repeatedly to stress this)**
...
AP_demres <-
   resilience(
     ...
     TDvector = FALSE,
     popname = "adelie penguin",
     verbose = FALSE, # set to TRUE to see the messages saying that either attenuation or amplification cannot be computed
     return.N = TRUE,
     return.t = TRUE
   )
...
This call returns an object of class `resil` that is a dataframe with 11 columns. 

```{r echo = TRUE}
colnames(AP_demres)
```

All 5 metrics are given and for all metrics (but damping ratio) we show the time step when the computed value of the metric is reached. Showing the time step when reactivity is reached is somewhat redundant (as per definition it is the population size at time step 1 after disturbance) but we decided to display it nevertheless, for consistency. Messages are displayed due to the fact that for some combinations of specified matrices and vectors the population either does not amplify or does not attenuate (they are disabled for the vignette for the sake of readability, but can be activated by setting the option `verbose = TRUE`. 

Given the initial demographic distribution specified by ${\textbf{n}_0}$, it is clear
...
First let's check when the damping ratio is at its minimum value
```{r, extremes}
...
The _time step 21_ is the year for which the damping ratio reached its _minimum value_ of 3.206. While the _year 7_ is the year with the highest value of damping ratio, which is _8.606_. This means that the population will converge to its long-term dynamics the fastest during this year. 
...
So we here see so-called overall or realised population trajectories in which both long-term and short-term dynamics are confounded. Each facet highlights the population trajectory for the year shown on the top. We see that in some years (like _year 14 and 15_) population is projected to increase rapidly whereas in a few years the populations are projected to decline ( _year 13_ ).

It is possible to order them in decreasing order of the population size projected at the last time step.
...
**VIK: I think this section on plotting is really nice. The only thing that is still missing is highlighting how inspecting the output that is either not standardised at all, or standardized in a specific way is useful for conservation/management. I feel it is less useful just to list the options that things can be done with no standardisation, or with one or the other standardisation. Explaining why that is useful, especially when comparing these outputs is what is valuable in this approach** 
```{r, echo = -1, fig.width = 6, fig.height = 6}
plot(AP_demres,
     listA = adeliepenguin,
     vector = vector_list,
...
However, `demres` also allows to use historically informed initial vectors, i.e. directly derived from observations in the field. 

# Initial vectors {#initial-vectors}
## Historically informed
The first advantage of `demres`is its adaptability to historically informed data from the field. If researchers have the information on the abundance of each respective (st)age class for each time step of the study, it is possible to integrate it in the framework.
...
If no empirical values are available, it is possible to simulate those that are reflecting possible situations of interest (e.g. reintroduction when mainly individuals of a specific age stage(s) are released) and investigate their effects on population trajectories. 
...
We again set `verbose = FALSE` to avoid the display all the messages and thus make the vignette better readable.
...
We can see that when using time-varying demographic vector while projecting the initial population vector forward, the population is more likely to amplify than it was when using the initial demographic distribution we previously provided (that was applied to each time step). We also see that the values of all resilience metrics are increasing rapidly over time and by the the year 28 we have population sizes in thousands as opposed to population sizes being below 100 when relying on the user-supplied population vectors. **VIK: I think it would be good in two words here to say what is the reason for this behaviour. Otherwise the reader may be confused by such a behaviour and think this is all wrong?**
